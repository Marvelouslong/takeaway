...
@WebServlet("/picturedriver_license")
public class photodriver_licenseServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private photoServiceImpl fbpi = new photoServiceImpl(); //调用service

    public photodriver_licenseServlet() {
        super();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        byte[] picturedriver_license = fbpi.picturedriver_license(Integer.parseInt(id)); //获得bookPicture
        response.setContentType("image/jpg");  //设置图片格式
        OutputStream out = response.getOutputStream(); //打开输出流
        out.write(picturedriver_license);  //输出图片
        out.flush();	//输出
        out.close();  //关闭输出
    }
}
