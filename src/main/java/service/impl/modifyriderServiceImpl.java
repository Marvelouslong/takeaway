...
public class modifyriderServiceImpl implements modifyriderService {
    ...
    public Boolean modifyrider(rider Rider, administrators admin) {
        Boolean flag=false;
        Connection connection=null;
        try {
            connection= BaseDao.getConnection();
            connection.setAutoCommit(false);//开启JDBC事务
            int updateNum = modifyriderdao.modifyrider(connection, Rider,admin);//执行修改sql
    ...
    public Boolean modifycontext(rider Rider) {
        Boolean flag=false;
        Connection connection=null;
        try {
            connection= BaseDao.getConnection();
            connection.setAutoCommit(false);//开启JDBC事务
            int updateNum = modifyriderdao.modifycontext(connection, Rider);//执行修改sql
            connection.commit();//提交事务
            if(updateNum>0){
                flag=true;
                System.out.println("yes");
